      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-b-xl shadow-2xl p-8 border-4 border-black border-t-0">
          <div className="text-center">
            <img src={pokemon.sprites.front_default} alt={pokemon.name} className="w-48 h-48 mx-auto" />
            <h2 className="text-gray-700 text-4xl font-bold capitalize mt-4">{pokemon.name}</h2>
            <div className="flex gap-2 justify-center mt-4">
              {pokemon.types.map((type) => (
                <span key={type.type.name} className="px-4 py-2 bg-green-500 text-white rounded-full font-semibold">
                  {type.type.name}
                </span>
              ))}
            </div>
